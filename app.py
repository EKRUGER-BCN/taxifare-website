...
.sl::before { content:''; display:block; width:12px; height:2px; background:var(--yellow); flex-shrink:0; }
...
/* tighten date/time widget spacing */
[data-testid="stDateInput"], [data-testid="stTimeInput"] { margin-bottom: 0 !important; }
[data-testid="stNumberInput"] { margin-bottom: 0 !important; }

/* ── SLIDER ── */
[data-testid="stSlider"] { padding-top: 0.2rem !important; }
...
.stButton > button:hover {
    ...
    box-shadow: 0 6px 24px rgba(247,201,72,0.3) !important;
}
...
.stat-box { background:var(--surface2); border:1px solid var(--border); border-radius:4px; padding:0.45rem; text-align:center; }
.stat-val { font-family:'Bebas Neue',sans-serif; font-size:1.1rem; color:var(--yellow); line-height:1; }
.stat-lbl { font-family:'Space Mono',monospace; font-size:0.42rem; color:var(--muted); letter-spacing:2px; text-transform:uppercase; margin-top:1px; }

/* ── FARE ── */
.fare-box {
    background:#08080f; border:1px solid var(--yellow); border-radius:6px;
    padding:1rem; text-align:center; position:relative; overflow:hidden; margin-top:0.6rem;
}
.fare-box::before {
    content:''; position:absolute; inset:0;
    background:radial-gradient(ellipse at center, rgba(247,201,72,0.07) 0%, transparent 70%);
}
.fare-lbl { font-family:'Space Mono',monospace; font-size:0.5rem; letter-spacing:4px; color:var(--muted); text-transform:uppercase; }
.fare-amt { font-family:'Bebas Neue',sans-serif; font-size:3rem; color:var(--yellow); line-height:1; text-shadow:0 0 30px rgba(247,201,72,0.5); letter-spacing:3px; }
.fare-cur { font-family:'Space Mono',monospace; font-size:0.55rem; color:var(--muted); letter-spacing:3px; }
.verdict {
    display:inline-block; margin-top:0.6rem; padding:3px 14px;
    border-radius:2px; font-family:'Space Mono',monospace; font-size:0.58rem;
...
.tip-total-lbl { font-family:'Space Mono',monospace; font-size:0.5rem; color:var(--muted); letter-spacing:2px; text-transform:uppercase; }
.tip-total-val { font-family:'Bebas Neue',sans-serif; font-size:1.5rem; color:var(--yellow); }

/* ── MAP ── */
[data-testid="stDeckGlJsonChart"] { border-radius:0 !important; border:none !important; }
...
[data-testid="column"]:first-child {
    ...
    padding: 0.9rem 1rem 1.5rem 1rem !important;
...
h3 {
    ...
    letter-spacing:2px !important; margin-bottom:0.2rem !important;
}
...
<div class="topbar">
    ...
    <div style="font-family:'Space Mono',monospace;font-size:0.5rem;color:#F7C948;letter-spacing:2px;">
        🚕 MANHATTAN→BROOKLYN FROM $12 &nbsp;·&nbsp; ✈️ JFK FLAT RATE $52 &nbsp;·&nbsp; 🌃 LATE NIGHT SURCHARGE AFTER 8PM
    </div>
...
with left:
    ...
    c1, c2 = st.columns(2)
    with c1:
        pickup_date = st.date_input("Date", value=datetime.today(), label_visibility="visible")
    with c2:
        pickup_time = st.time_input("Time", value=datetime.now().time(), label_visibility="visible")

    hour = pickup_time.hour
    if 7 <= hour <= 9 or 17 <= hour <= 19:
        tctx, tclr = "⚠️ Rush hour — higher fares", "#ff9900"
    elif 22 <= hour or hour <= 5:
        tctx, tclr = "🌙 Late night surcharge active", "#8888ff"
    else:
...
""", unsafe_allow_html=True)
        ...
        st.markdown('<div class="sl" style="margin-top:0.6rem;">Tip</div>', unsafe_allow_html=True)
        st.markdown(f"""
<div class="stats-row">
...
<div class="tip-total">
    <div class="tip-total-lbl">Total w/ 20% tip</div>
    <div class="tip-total-val">${fare + t20:.2f}</div>
</div>
""", unsafe_allow_html=True)
...
with right:
    ...
    scatter_layer = pdk.Layer(
        "ScatterplotLayer",
        data=[
            {"pos": [pickup_lon, pickup_lat],   "col": [247, 201, 72, 220], "lbl": "📍 Pickup"},
            {"pos": [dropoff_lon, dropoff_lat], "col": [255, 80, 80, 220],  "lbl": "🏁 Dropoff"},
        ],
        get_position="pos",
        get_fill_color="col",
        get_radius=80,
        radius_scale=6, radius_min_pixels=7, radius_max_pixels=18,
    ...
    # height matches viewport minus topbar
    st.pydeck_chart(deck, use_container_width=True, height=740)
